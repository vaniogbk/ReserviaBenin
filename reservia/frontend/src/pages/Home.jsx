    ...
    <div className="pt-16">
      ...
      <section className="relative min-h-[85vh] flex items-center overflow-hidden"
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-8 py-20">
          <div className="inline-block bg-terracotta/10 border border-terracotta/30 text-terracotta text-xs font-semibold tracking-[2px] uppercase px-4 py-2 rounded-full mb-6">
            Plateforme de réservation — Bénin
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-8 py-20">
        <div className="flex items-end justify-between mb-10">
          ...
          <Link to="/hebergements" className="text-sm text-terracotta hover:underline font-medium">Voir tout →</Link>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {hebs?.data?.data?.map(h => <HebergementCard key={h.id} item={h} />) ??
           Array(3).fill(0).map((_, i) => (
            <div key={i} className="rounded-2xl bg-sand h-72 animate-pulse" />
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-8 pb-20">
        <div className="flex items-end justify-between mb-10">
          ...
          <Link to="/evenements" className="text-sm text-terracotta hover:underline font-medium">Voir tout →</Link>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {evts?.data?.data?.map(e => <EvenementCard key={e.id} item={e} />) ??
           Array(3).fill(0).map((_, i) => (
            <div key={i} className="rounded-2xl bg-sand h-72 animate-pulse" />
          ))}
        </div>
