...
const KPI = ({ icon, label, value, sub, color }) => (
  <div className="bg-white rounded-2xl p-6 shadow-sm">
    <div className="flex items-center justify-between mb-4">
      ...
      <span className={`badge text-xs font-semibold ${color}`}>{sub}</span>
    </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <KPI icon="💰" label="Revenus ce mois" value={`${revenusMois} FCFA`} sub="+12% ↑" color="bg-green-100 text-green-700" />
            <KPI icon="📋" label="Réservations actives" value={kpis?.reservations_actives} sub="en cours" color="bg-blue-100 text-blue-700" />
            <KPI icon="⭐" label="Note moyenne" value={kpis?.note_moyenne} sub="★ sur 5" color="bg-amber-100 text-amber-700" />
            <KPI icon="👥" label="Nouveaux utilisateurs" value={kpis?.utilisateurs_ce_mois} sub="ce mois" color="bg-purple-100 text-purple-700" />
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              ...
              <Link to="/admin/reservations" className="text-sm text-primary hover:underline">Voir tout →</Link>
            </div>
